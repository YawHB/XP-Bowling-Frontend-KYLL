...
export default function DinnertableForm({ addBooking, formattedDate }: DinnertableFormProps) {
  const [startTime, setStartTime] = useState<string>("10:00");
  const [tables, setLanes] = useState<number>(1);
  const dinnerTime: number = 2;
  const [endTime, setEndTime] = useState<string>(calculatedEndTime("10:00", dinnerTime));

  useEffect(() => {
    setEndTime(calculatedEndTime(startTime, dinnerTime));
  }, [startTime]);
  ...
  function calculatedEndTime(startTime: string, dinnerTime: number): string {
  ...
  return (
    <form onSubmit={handleSubmit} className="container mx-auto pr-4 py-8 max-w-screen-md">
      ...
      <div className="w-full pr-2">
        <button type="submit" className="w-32 mt-4 p-2 text-lg font-bold text-white whitespace-nowrap bg-green-500 hover:bg-green-600 active:bg-green-700 focus:outline-none focus:ring focus:ring-green-300 border-2 border-yellow-300">
          Tilføj
        </button>
      </div>
    </form>
